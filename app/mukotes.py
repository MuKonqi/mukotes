...
def first_start():
    ...
    def llangtr():
        ...
        settings()
    lwindow=Tk()
    lwindow.title("First Start of Mukotes")
    lwindow.config(background=bg)
    lwindow.resizable(0, 0)
    ltext1=Label(lwindow, background=bg, foreground=fg, font="arial 11 bold italic", text="Please choose a language for Mukotes.\nMukotes için lütfen bir dil seçin.")
    ltext1.pack()
...
def file():
    ...
    ftext1=Text(fwindow, background=fg, foreground=bg, yscrollcommand=fscroll.set)
    ...
    if name.get() == "":
        ...
        nf2=0
        if os.path.isfile(en):
            flabel1=Label(fwindow, background=bg, foreground=fg, font="arial 11 bold", text="You can start taking your note.\n\nBefore save this file you should type file name.")
        elif os.path.isfile(tr):
            flabel1=Label(fwindow, background=bg, foreground=fg, font="arial 11 bold", text="Notunuzu almaya başlayabilirsiniz.\n\nBu dosyayı kaydetmeden önce dosya adı girmelisiniz.")
        ...
        input_file.close()
        if os.path.isfile(en):
            flabel1=Label(fwindow, background=bg, foreground=fg, font="arial 11 bold", text="You can start editing your note.")
        elif os.path.isfile(tr):
            flabel1=Label(fwindow, background=bg, foreground=fg, font="arial 11 bold", text="Notunuzu düzenlemeye başlayabilirsiniz.")
    if os.path.isfile(en):
        if  nf == 0:
...
if os.path.isdir("/home/"+username+"/Notes"):
    ...
    text1=Text(window, background=fg, foreground=bg, yscrollcommand=scroll.set)
    text1.insert(END, out)
    text1.insert(END, err)
    ...
    if os.path.isfile(en):
        ...
        label1=Label(window, background=bg, foreground=fg, font="arial 11 bold italic", text="The notes taken are listed below.")
        label2=Label(window, background=bg, foreground=fg, font="arial 11 bold italic", text="To Open Existing Note: Just Type File Name\nTo Open New Note: Leave Box Empty")
        button1=Button(window, font="arial 11", cursor="hand2", activebackground=a_button_bg, activeforeground=a_button_fg, foreground=button_fg, borderwidth="3", background=button_bg, text="Open",command=file)
        button2=Button(window, font="arial 11", cursor="hand2", activebackground=a_button_bg, activeforeground=a_button_fg, foreground=button_fg, borderwidth="3", background=button_bg, text="Delete",command=delete)
        sobutton=Button(window, font="arial 11", cursor="hand2", activebackground=a_button_bg, activeforeground=a_button_fg, foreground=button_fg, borderwidth="3", background=button_bg, text="Settings",command=settings)
    elif os.path.isfile(tr):
        window.title("Notlar | Mukotes")
        label1=Label(window, background=bg, foreground=fg, font="arial 11 bold italic", text="Aşağıda alınan notlar listelenmiştir.")
        label2=Label(window, background=bg, foreground=fg, font="arial 11 bold italic", text="Var Olan Notu Açmak İçin: Sadece Dosya Adı Yazın\nYeni Not Açmak İçin: Kutuyu Boş Bırakın")  
        button1=Button(window, font="arial 11", cursor="hand2", activebackground=a_button_bg, activeforeground=a_button_fg, foreground=button_fg, borderwidth="3", background=button_bg, text="Aç",command=file)        
        button2=Button(window, font="arial 11", cursor="hand2", activebackground=a_button_bg, activeforeground=a_button_fg, foreground=button_fg, borderwidth="3", background=button_bg, text="Sil",command=delete)   
        sobutton=Button(window, font="arial 11", cursor="hand2", activebackground=a_button_bg, activeforeground=a_button_fg, foreground=button_fg, borderwidth="3", background=button_bg, text="Ayarlar",command=settings)     
